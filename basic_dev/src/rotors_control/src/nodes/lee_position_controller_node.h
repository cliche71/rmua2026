...
class LeePositionControllerNode {
 ...
 private:
  ...
  ros::Publisher disturbance_estimate_pub_;
  ros::Publisher torque_disturbance_estimate_pub_;

  EigenTrajectoryPointDeque commands_;
  std::deque<ros::Duration> command_waiting_times_;
  ros::Timer command_timer_;
  bool use_airsim_frame_adapter_;
  bool got_initial_pose_;
  std::string command_pose_topic_;
  std::string command_trajectory_topic_;
  std::string odometry_topic_;
  std::string initial_pose_topic_;
  std::string motor_topic_;
  std::string disturbance_estimate_topic_;
  std::string torque_disturbance_estimate_topic_;
  ros::Time last_odometry_stamp_;
  Eigen::Matrix4d t_world_flu_initial_;
  Eigen::Matrix4d t_world_flu_initial_inv_;
  Eigen::Matrix4d t_flu_ned_;
  ...
  void InitialPoseCallback(const geometry_msgs::PoseStampedConstPtr& pose_msg);
  bool TransformTrajectoryPointFromAirsim(EigenTrajectoryPoint* trajectory_point) const;
  bool TransformOdometryFromAirsim(EigenOdometry* odometry) const;
};
}
